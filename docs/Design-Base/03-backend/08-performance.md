...
```python
# ❌ async 函式內呼叫 sync 重 IO / CPU(本專案用 argon2id)
async def hash_password(plain: str) -> str:
    return _pwd_hasher.hash(plain)  # argon2-cffi,同步 + CPU 重

# ✅ 走 to_thread
async def hash_password(plain: str) -> str:
    return await asyncio.to_thread(_pwd_hasher.hash, plain)
```

阻塞操作清單(都需 `to_thread`):

- `argon2-cffi` 的 `PasswordHasher.hash` / `.verify`(本專案密碼雜湊)
- `Pillow` / `numpy` 大計算
