...
- **回應格式**:後端以 **SSE**(`text/event-stream`)回傳,但**不**原樣轉發 OpenRouter chunk —— 比照非串流端點的精簡原則(見 [§ 6](#6-請求改寫與欄位過濾)),逐 chunk 解析後**只回 `{ id, content }`**:`data: {"id":"<gen-id>","content":"<本段文字>"}\n\n` … 最後 `data: [DONE]\n\n`。OpenRouter 原始欄位(`provider` / `cost` / `model` 路由 / `role` / `finish_reason` / `usage` 等)**一律不外露**(避免洩漏成本與供應商等內部資訊);`: OPENROUTER PROCESSING` keep-alive 註解與無文字的 chunk(role 宣告 / 結束 / usage)不轉發。串流端點**不**套用統一 `ApiResponse` 包裝。
...
- **不可靜默截斷**:OpenRouter 串流中途自身的 error(無 content 的 `data:` chunk)**必須**轉成簡化的 `data: {"error":"upstream_error"}` 事件(**不**外露原始錯誤明細);若為「後端側」與上游的連線無預警中斷,後端**必須**補送 `data: {"error":"openrouter_unavailable"}` 後再 `data: [DONE]` 收尾。兩者皆**禁止**靜默截斷。
