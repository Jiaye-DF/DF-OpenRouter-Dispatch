...
>
> **更新原則**:新增 / 棄用任一規範檔 → **同步**更新本檔(否則 agent 會漏載)。本檔與 `AGENTS.md § Just-in-time Loading` 必一致。
>
> **本專案特有檔**(`*/90-project-*.md`、`90-third-party-service/50-openrouter.md`)為 DF-OpenRouter-Dispatch 在 HE 通用層之外的補充,見文末〈本專案特有檔〉。
...
| `01-versions.md` | 版本鎖到 patch + 套件清單 + 強制鎖定 Next 14.2 / React 18.3 / Tailwind v4 / Python 3.14(已 re-baseline) |
...
| `03-passwords-and-pii.md` | argon2id(`argon2-cffi`)+ asyncio.to_thread + PII 隔離 |
...
3. **本檔與 `AGENTS.md § Just-in-time Loading`**:同源,任一更新另一同步
4. 任何 PR 改動 `docs/Design-Base/*` 結構 → reviewer 必檢查本檔是否同步

---

## 本專案特有檔(DF-OpenRouter-Dispatch,HE 通用層之外)

> 2026-06-25 由原扁平 Design-Base 遷入。命名慣例:area 內 `90-project-*.md` 為該領域的**專案專屬補充**(與同資料夾 HE 通用檔疊加,衝突時專案檔更嚴格者勝,但**不得**降低 HE 地板)。

| 檔 | 用途 | 何時讀 |
| --- | --- | --- |
| `00-overview/90-project-overview.md` | 專案目標 / 技術棧 / Monorepo / 服務組成 / 資料流 | 啟動任務先掃 |
| `00-overview/91-project-naming-env.md` | 命名慣例 + `.env` 分區 + Git 流程(本專案) | 改 env / 命名 |
| `01-propose/90-project-task-spec.md` | Task 產出格式 + 對齊章節 + OpenRouter/稽核/UID 檢核 | 寫 propose / tasks |
| `02-frontend/90-project-frontend.md` | Header/NavBar/主題/Card/Dialog/API client(本專案 UI) | 前端任務 |
| `02-frontend/91-project-ui-ux.md` | UI/UX 細節規範 | 前端樣式 |
| `03-backend/90-project-backend.md` | ApiResponse 外殼 / 錯誤訊息 / 分層 / 路由命名(本專案) | 後端任務 |
| `03-backend/91-project-auth.md` | 本地登入 / JWT / 失效清單 / 密碼規則 | 認證 |
| `03-backend/92-project-permission.md` | 管理端 / 代理端分離 + 角色 + 配額 + 稽核(HE 無對應) | 權限 |
| `04-databases/90-project-database.md` | 必備欄位 / pid-uid / 軟刪除 / Snowflake / baseline migration 補救 | DB 任務 |
| `06-Coolify-CD/90-project-deployment.md` | 本專案 compose / SERVICE_URL / 變數注入 | 部署 |
| `90-third-party-service/50-openrouter.md` | **模型代理整合**(OpenRouter + Internal、雙因子、串流、重試、用量、速率) | 任何代理 / 模型呼叫 |
