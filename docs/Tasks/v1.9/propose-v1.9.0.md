...
> 4. **重複送出限制:本版不限制(可重複送)**;去重規則(是否去重、以何鍵去重)**移至 v1.9.1** 一併設計,本版資料表不因此改 schema。
>
> 其餘隨之確立(無異議):**單一 GET 端點依角色分流**(§ 5.2)、**送出後不發通知**(通知屬審核流程,隨 #1 移至 v1.9.1)。

### 移至 v1.9.1 設計

- **審核流程**(核准 / 駁回 / 退回補件 / 狀態流轉 / 通知)。
- **重複送出去重規則**(決議 #4):是否去重、以何鍵去重(例如同 `department_code` + `project_name` 不可同時存在 pending),於 v1.9.1 一併評估設計。
