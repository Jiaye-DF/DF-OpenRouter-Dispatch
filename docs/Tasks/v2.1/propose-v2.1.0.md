...
### 6.2 視覺形式(user 授權 agent 決策 → 2026-06-26 改版定版:分層 + Dialog + 可追溯)

> 初版定「卡片並排、輸出一律 render」,user 反映**難閱讀又佔版面**,改為**分層顯示(平常精簡、要看輸出再點開)**並補可追溯資訊。下為定版,視覺實作以此為準。

- **第一層 — 頁頂統計列**:`stats` 一排小指標卡——總筆數 / 建議維持 / 建議改用 / 平手 / 未裁決 / 失敗。
- **第二層 — 每筆用量紀錄一列(分組,可展開,精簡非大卡片)**:
  - 收合列:原模型 key + **對應 usage_log uid(純文字顯示 + 複製,嚴禁做成連結)** + 彙總裁決 Badge(如「2/3 建議維持」)+ 執行時間 + 展開鈕。
  - 展開:該組推薦模型的**精簡表格**(每個推薦模型一列)——`推薦模型 key | 推薦者(誰推薦的) | 裁決 Badge | 信心 | 成本Δ(綠省紅貴) | 延遲 | [查看] 鈕`。**此層不 render 輸出原文**。
- **第三層 — Dialog(點 [查看] 開啟)**:該推薦模型的完整對照——對應 usage_log uid + 推薦者 + 裁決/信心;**任務輸入原文**;**原模型輸出 vs 該推薦模型輸出並排**(桌機並排 / 手機堆疊,各 `max-h` 捲動);裁決理由 + 裁決模型。失敗列亦可開(顯示 error_code + 輸入 + 原模型輸出供對照)。
- **可追溯欄位(讀取層 derive,不動 DB)**:`RerunGroup.original_input_text`(任務輸入)、`RerunRecommendation.recommended_by`(由哪個評審推薦,從 candidate 反查,失敗/未裁決亦可得)。
- **RWD**:Mobile-first;第二層表格手機橫捲或堆疊;Dialog 並排輸出手機堆疊;觸控目標 ≥ 44px。
- **功能需求(不可妥協)**:① 依用量紀錄分組;② 標出對應 usage_log uid;③ 每個推薦模型標「推薦者」;④ 點開 Dialog 看任務輸入 + 原 vs 推薦模型**真實輸出原文**;⑤ 成本Δ + 裁決 + 信心;⑥ 頁頂裁決分布統計;⑦ **不跳轉用量紀錄**(uid 僅顯示不連結)。
