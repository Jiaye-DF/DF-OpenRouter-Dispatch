...
**Phase 2(`--phase rewrite`)是本版唯一不可逆的操作。**
...
- Phase 1(`--phase upload`)則完全安全:它對 DB 只有 SELECT,跑到一半中止零副作用,可以放心先跑。
...
```bash
CMD="docker compose -f docker-compose-prod.yml exec backend python scripts/migrate_base64_to_s3.py"
```

### 3-1 Phase 1 · 只上傳(DB 零變更)

```bash
# (a) 先 dry-run:只報數字,不上傳、不寫 DB
$CMD --phase upload --dry-run

# (b) 小範圍試跑:只處理前 50 列,確認 key / 內容正確
$CMD --phase upload --limit 50

# (c) 全量上傳
$CMD --phase upload --batch-size 200
```
...
### 3-3 Phase 2 · 改寫 JSONB(不可逆)

```bash
# (a) 必跑 dry-run:報「可改寫 N / 待改寫」,不送出任何 UPDATE
$CMD --phase rewrite --dry-run

# (b) 小範圍實跑:先改 20 列,到明細頁確認圖片顯示正常
$CMD --phase rewrite --limit 20

# (c) 全量改寫
$CMD --phase rewrite --batch-size 200
```
...
因此:**Ctrl-C 中斷後,原指令直接重跑即可。** 只有想加速時才需要 `--after-pid`:

```bash
$CMD --phase rewrite --after-pid 123456 --batch-size 200
```
...
| 原因代碼 | 意義 | 處置 |
| --- | --- | --- |
| `s3_object_missing` | **安全網生效**:重算出的 key 在 S3 不存在 → 整列未改寫 | 回去補跑 Phase 1(`--phase upload`),再重跑 Phase 2。若補跑後仍缺,檢查 `S3_KEY_PREFIX` / `S3_BUCKET` 是否與 Phase 1 一致 |
...
| 參數 | 說明 |
| --- | --- |
| `--phase upload\|rewrite` | 必填。`upload` = 只上傳(DB 零變更);`rewrite` = 改寫 JSONB(**不可逆**) |
| `--dry-run` | 只報統計:Phase 1 不上傳、Phase 2 不送出任何 UPDATE |
| `--batch-size N` | 每批撈幾列(`pid` 游標),預設 200;Phase 2 每批一個 transaction |
| `--limit N` | 最多處理幾列,`0` = 不限。小範圍試跑用 |
| `--after-pid N` | 從此 `pid` 之後開始(續跑加速用;正常重跑不需要) |
| `--concurrency N` | **僅 Phase 1**:同時進行的 S3 呼叫數上限,預設 1(循序) |
